...
async function runPerformanceTests() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(
      `   Payload size: ${JSON.stringify(scenario.body).length} bytes\n`,
    );
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      if (result.success) {
        console.log(
          ` ✓ ${result.duration.toFixed(2)}ms${result.attempt > 1 ? ` (${result.attempt} attempts)` : ""}`,
        );
      } else {
    ...
    if (retries.length > 0 || failedResults.length > 0) {
      metrics.retryMetrics[scenario.name] = {
        ...
        failureRate:
          ((failedResults.length / results.length) * 100).toFixed(1) + "%",
      };
      ...
      console.log(
        `      Retry rate: ${((retries.length / results.length) * 100).toFixed(1)}%`,
      );
      console.log(
        `      Failure rate: ${((failedResults.length / results.length) * 100).toFixed(1)}%`,
      );
    }
  ...
  Object.entries(metrics.apiLatency).forEach(([scenario, stats]) => {
    ...
    if (p95Value > 500) {
      console.log(
        `⚠️  ${scenario}: P95 latency (${stats.p95}ms) exceeds 500ms target`,
      );
      console.log(
        `   Consider implementing request caching or optimizing the endpoint`,
      );
    }

    if (p99Value > p95Value * 2) {
      console.log(
        `⚠️  ${scenario}: High variance detected (P99 is ${(p99Value / p95Value).toFixed(1)}x P95)`,
      );
      console.log(
        `   Consider implementing request timeout and circuit breaker`,
      );
    }
  });
  ...
  const outputPath = path.join(
    __dirname,
    `../docs/api-performance-${timestamp}.json`,
  );
