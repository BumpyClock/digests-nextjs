...
async function measureMemoryUsage() {
  ...
  console.log(`  After feeds: ${(afterFeedsMemory.heapUsed / 1024 / 1024).toFixed(2)}MB`);
  console.log(`  After multiple: ${(afterMultipleMemory.heapUsed / 1024 / 1024).toFixed(2)}MB`);
}
...
async function measureBundleSize() {
  ...
  try {
    ...
    const apiServiceSize = (await fs.stat(path.join(__dirname, '../services/api-service.ts'))).size;
    const typeGuardsSize = (await fs.stat(path.join(__dirname, '../utils/type-guards.ts'))).size;
    const securitySize = (await fs.stat(path.join(__dirname, '../utils/security.ts'))).size;
...
async function measureCacheEfficiency() {
  ...
  const urls = [TEST_URLS[0], TEST_URLS[0], TEST_URLS[1], TEST_URLS[0], TEST_URLS[2]];
  ...
  metrics.cachePerformance.hitRate = (metrics.cachePerformance.hits / total) * 100;
...
async function generateReport() {
  ...
  const report = {
    ...
    summary: {
      avgColdLatency: metrics.apiLatency.filter(m => m.type === 'cold').reduce((sum, m) => sum + m.latency, 0) / metrics.apiLatency.filter(m => m.type === 'cold').length || 0,
      avgWarmLatency: metrics.apiLatency.filter(m => m.type === 'warm').reduce((sum, m) => sum + m.latency, 0) / metrics.apiLatency.filter(m => m.type === 'warm').length || 0,
      memoryGrowth: metrics.memoryUsage.length > 1 ? 
        ((metrics.memoryUsage[metrics.memoryUsage.length - 1].heapUsed - metrics.memoryUsage[0].heapUsed) / metrics.memoryUsage[0].heapUsed * 100).toFixed(1) + '%' : 
        'N/A',
      cacheHitRate: metrics.cachePerformance.hitRate.toFixed(1) + '%',
      bundleSizeImpact: metrics.bundleSize ? `${(metrics.bundleSize.total / 1024).toFixed(1)}KB` : 'Unknown'
    },
    details: metrics
  };
  ...
  console.log('\\n📊 Summary:');
  console.log(`  Average cold latency: ${report.summary.avgColdLatency.toFixed(2)}ms`);
  console.log(`  Average warm latency: ${report.summary.avgWarmLatency.toFixed(2)}ms`);
...
module.exports = { measureApiLatency, measureMemoryUsage, measureCacheEfficiency };