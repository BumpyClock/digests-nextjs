...
import { useFeeds, feedsKeys, type FeedsQueryData } from '@/hooks/queries/use-feeds'
import { useAddFeed, useDeleteFeed, useUpdateFeed } from '@/hooks/queries/use-feed-mutations'
...
const createMockStore = (feeds: Feed[] = [], items: FeedItem[] = []) => ({
  ...
  sortFeedItemsByDate: jest.fn().mockImplementation((items: FeedItem[]) => 
...
describe('Data Integrity Tests', () => {
  ...
  describe('Cross-Store Data Consistency', () => {
    ...
    it('should preserve read status across migrations', async () => {
      ...
      mockUseFeedStore.mockReturnValue(zustandStore)
      mockApiService.refreshFeeds.mockResolvedValue({ feeds: [feed], items: [item] })
  ...
  describe('Mutation Data Integrity', () => {
    it('should maintain data integrity during optimistic updates', async () => {
      const existingFeed = createMockFeed('feed-1', 'https://blog1.com/feed.xml')
      ...
      mockApiService.feeds.create.mockReturnValue(apiPromise as any)
      ...
      const optimisticFeed = feedsResult.current.feeds.find(f => f.guid.startsWith('temp-'))
    ...
    it('should rollback optimistic updates on error', async () => {
      ...
      // Mock API error
      mockApiService.feeds.create.mockRejectedValue(new Error('API Error'))

      await act(async () => {
        try {
          await addResult.current.mutateAsync({ url: 'https://invalid.com/feed.xml' })
        } catch (error) {
          // Expected to fail
        }
      })
    ...
    it('should handle concurrent mutations safely', async () => {
      ...
      // Mock API responses
      mockApiService.feeds.create.mockResolvedValue(feed2)
      mockApiService.refreshFeeds.mockResolvedValue({
        feeds: [feed2],
        items: []
      })
      ...
      const deletePromise = deleteResult.current.mutateAsync(existingFeed.feedUrl)
  ...
  describe('Data Persistence and Recovery', () => {
    ...
    it('should merge conflicting data correctly', () => {
      ...
      const remoteData: Partial<PersistedFeedData> = {
        feeds: [createMockFeed('feed-1', 'https://blog.com/feed.xml'), createMockFeed('feed-2', 'https://blog2.com/feed.xml')],
        items: [createMockItem('item-1', 'https://blog.com/feed.xml'), createMockItem('item-2', 'https://blog2.com/feed.xml')],
    ...
    it('should clean up old items correctly', () => {
      ...
      const data: PersistedFeedData = {
        feeds: [createMockFeed('feed-1', 'https://blog.com/feed.xml')],
        items: [
          { ...createMockItem('old-item', 'https://blog.com/feed.xml'), published: oldDate },
          { ...createMockItem('recent-item', 'https://blog.com/feed.xml'), published: recentDate }
        ],
        lastFetched: now,
        version: 3,
  ...
  describe('Error Recovery and Resilience', () => {
    it('should recover from API failures', async () => {
      ...
      // First call fails
      mockApiService.refreshFeeds.mockRejectedValueOnce(new Error('Network error'))
    ...
    it('should handle partial failures in batch operations', async () => {
      ...
      mockApiService.feeds.create
        .mockResolvedValueOnce(createMockFeed('feed-1', 'https://blog1.com/feed.xml'))
        .mockRejectedValueOnce(new Error('Invalid feed'))
        .mockResolvedValueOnce(createMockFeed('feed-3', 'https://blog3.com/feed.xml'))
