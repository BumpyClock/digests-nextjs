...
const FeedManager: React.FC = () => {
  ...
  const [readerView, setReaderView] = React.useState<ReaderViewResponse | null>(null)
  ...
  return (
    <div>
      ...
      <button onClick={() => addFeed('http://example.com/new-feed.xml')}>Add Feed</button>
...
describe('API Service Integration', () => {
  ...
  describe('Feed Fetching', () => {
    ...
    it('should load and display feeds on mount', async () => {
      ...
      expect(screen.getByTestId('feed-feed-1')).toHaveTextContent('Tech Blog RSS')
      expect(screen.getByTestId('feed-feed-2')).toHaveTextContent('Science Daily Feed')

      // Check items are displayed
      expect(screen.getByTestId('item-item-1')).toHaveTextContent('Breaking Tech News')
    ...
    it('should refresh feeds when refresh button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith('Feeds refreshed successfully')
      })
    ...
    it('should add a new feed', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('feed-feed-3')).toHaveTextContent('New Blog Feed')
      })
  ...
  describe('Reader View', () => {
    const mockReaderView: ReaderViewResponse = {
      ...
      status: 'success',
      content: '<h1>Article Title</h1><p>This is the article content in reader mode.</p>',
      title: 'Article Title',
      siteName: 'Tech Blog',
    ...
    it('should load reader view when article is clicked', async () => {
      ...
      expect(screen.getByTestId('reader-view')).toHaveTextContent('Article Title')
      expect(screen.getByTestId('reader-view')).toHaveTextContent('This is the article content in reader mode.')
    })
  ...
  describe('Cache Behavior', () => {
    it('should use cached data on subsequent requests', async () => {
      ...
      // First request
      const result1 = await apiService.fetchFeeds(['http://example.com/feed1.xml'])
      expect(mockFetch).toHaveBeenCalledTimes(1)

      // Second request - should use cache
      const result2 = await apiService.fetchFeeds(['http://example.com/feed1.xml'])
    ...
    it('should bypass cache when refreshing', async () => {
      ...
      const refreshResult = await apiService.refreshFeeds(['http://example.com/feed1.xml'])
      expect(mockFetch).toHaveBeenCalledTimes(2)
  ...
  describe('Error Handling', () => {
    it('should handle different HTTP error codes', async () => {
      ...
      for (const { status, message } of errorCases) {
        ...
        await expect(apiService.fetchFeeds(['http://example.com/feed.xml']))
    ...
    it('should handle network timeouts', async () => {
      ...
      const fetchPromise = apiService.fetchFeeds(['http://example.com/feed.xml'])
  ...
  describe('Offline Mode', () => {
    it('should return cached data when offline', async () => {
      ...
      const onlineResult = await apiService.fetchFeeds(['http://example.com/feed1.xml'])
      
      // Simulate offline - network error
      mockFetch.mockRejectedValueOnce(new Error('Network error'))

      // Should still get cached data
      const offlineResult = await apiService.fetchFeeds(['http://example.com/feed1.xml'])
