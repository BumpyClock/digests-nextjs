...
// Define as a getter/setter so it can be reassigned
...
// Make sure the onLine property is writable
...
describe("Offline Sync Tests", () => {
  ...
  describe("Offline Queue Management", () => {
    it("should queue operations when offline", async () => {
      ...
      expect(result.current.queue[0].mutationKey).toEqual([
        "ADD_FEED",
        undefined,
      ]);
    ...
    it("should retry failed operations", async () => {
      ...
      const mockCreate = mockApiService.feeds.create as jest.MockedFunction<
        typeof mockApiService.feeds.create
      >;
      mockCreate
        .mockRejectedValueOnce(new Error("Network error"))
        .mockResolvedValueOnce(
          createMockFeed("retry-feed", "https://retry.com/feed.xml"),
        );
  ...
  describe("Data Synchronization and Conflict Resolution", () => {
    it("should handle conflicts between local and remote data", async () => {
      ...
      await waitFor(() => {
        expect(result.current.feeds[0]?.feedTitle).toBe(
          "Updated Title (Remote)",
        );
      });
    });
