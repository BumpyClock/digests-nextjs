...
describe("API Service Performance", () => {
  ...
  describe("Direct API Call Performance", () => {
    ...
    it("should demonstrate cache performance benefits", async () => {
      ...
      console.log(
        `Speed improvement: ${(uncachedTime / cachedStats.avg).toFixed(1)}x`,
      );
  ...
  describe("Concurrent Request Performance", () => {
    it("should handle concurrent requests efficiently", async () => {
      ...
      const urls = Array.from(
        { length: 5 },
        (_, i) => `http://example.com/feed-${i}.xml`,
      );
      ...
      await Promise.all(urls.map((url) => apiService.fetchFeeds([url])));
      ...
      console.log(
        `Speed improvement: ${(sequentialTime / concurrentTime).toFixed(1)}x`,
      );
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        results.push({
          operation:
            operations[i]
              .toString()
              .match(/apiService\.(\w+(?:\.\w+)?)/)?.[1] || "unknown",
          time,
        });
      }
...
describe("Performance Comparison: Direct API vs Worker Simulation", () => {
  ...
  const simulateWorkerDelay = async <T>(
    operation: () => Promise<T>,
  ): Promise<T> => {
