...
test.describe("Feed Management Workflows", () => {
          ...
          items: [
            {
              ...
              description:
                "Learn how to manage server state effectively with React Query",
  ...
  test("complete feed management lifecycle", async ({ page }) => {
    ...
    await test.step("Add new feed", async () => {
      ...
      await expect(
        page.locator('[data-testid="feed-test-feed-1"]'),
      ).toBeVisible();
    ...
    await test.step("View feed items", async () => {
      ...
      await expect(
        page.locator("text=Getting Started with React Query"),
      ).toBeVisible();
      await expect(page.locator("text=Jane Developer")).toBeVisible();

      // Check that article thumbnail is displayed
      await expect(
        page.locator('img[alt*="Getting Started with React Query"]'),
      ).toBeVisible();
    });
    ...
    await test.step("Read article", async () => {
      ...
      await expect(
        page.locator('[data-testid="article-reader"]'),
      ).toBeVisible();
      await expect(
        page.locator("text=React Query is a powerful library"),
      ).toBeVisible();

      // Verify article is marked as read
      await expect(
        page.locator('[data-testid="article-1"][data-read="true"]'),
      ).toBeVisible();
    });
    ...
    await test.step("Refresh feed", async () => {
      ...
      await page.click(
        '[data-testid="feed-test-feed-1"] button[aria-label="Open menu"]',
      );
    ...
    await test.step("Test offline functionality", async () => {
      ...
      await expect(
        page.locator("text=Changes will be synced when connection is restored"),
      ).toBeVisible();
  ...
  test("error handling workflow", async ({ page }) => {
    await test.step("Handle invalid feed URL", async () => {
      ...
      await page.fill(
        'input[name="url"]',
        "https://invalid-feed-url.com/not-a-feed",
      );
      await page.click('button[type="submit"]');
  ...
  test("search and filter workflow", async ({ page }) => {
    ...
    await test.step("Filter by feed", async () => {
      ...
      await expect(
        page.locator(
          '[data-testid="article-list"] [data-feed-id="test-feed-1"]',
        ),
      ).toBeVisible();
  ...
  test("reading preferences workflow", async ({ page }) => {
    ...
    await test.step("Mark articles as read/unread", async () => {
      ...
      await page.click(
        '[data-testid="article-1"] [data-testid="mark-read-button"]',
      );
      ...
      // Mark as unread
      await page.click(
        '[data-testid="article-1"] [data-testid="mark-unread-button"]',
      );

      // Verify unread status
      await expect(
        page.locator('[data-testid="article-1"][data-read="false"]'),
      ).toBeVisible();
    });
    ...
    await test.step("Add to read later", async () => {
      // Add to read later
      await page.click(
        '[data-testid="article-1"] [data-testid="read-later-button"]',
      );
      ...
      await page.click(
        '[data-testid="article-1"] [data-testid="remove-read-later-button"]',
      );
    ...
    await test.step("Mark all as read", async () => {
      ...
      await expect(
        page.locator('[data-testid="article-list"] [data-read="false"]'),
      ).toHaveCount(0);
    });
  });
  ...
  test("performance and caching workflow", async ({ page }) => {
    ...
    await test.step("Test background refresh", async () => {
      ...
      await expect(
        page.locator('[data-testid="background-sync-indicator"]'),
      ).toBeVisible();
    });
  });
});
