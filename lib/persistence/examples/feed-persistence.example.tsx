...
export const useReadStatus = () => {
  ...
  const readItems = useUIStore(state => state.readItems)
...
export function OfflineIndicator() {
  const { isOnline, lastSync, isStale } = useOfflineStatus()
...
function mergeFeedData(
  ...
  const newFeeds = (newData.feeds || []).filter(f => !existingFeedUrls.has(f.feedUrl))
  const newItems = (newData.items || []).filter(i => !existingItemIds.has(i.id))
