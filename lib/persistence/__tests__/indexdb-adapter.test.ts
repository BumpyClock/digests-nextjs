...
describe('IndexedDBAdapter', () => {
  ...
  describe('Batch Operations', () => {
    it('should get multiple values at once', async () => {
      ...
      const results = await adapter.getMany<{ value: number }>(['key1', 'key2', 'key3', 'non-existent'])
  ...
  describe('Error Handling', () => {
    ...
    it('should handle quota exceeded errors with cleanup', async () => {
      ...
      await expect(adapter.set('new-key', { data: 'small' })).resolves.not.toThrow()
    })
  })
  ...
  describe('Performance', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      const largeObject = {
        data: new Array(1000).fill({ id: 1, name: 'test', value: Math.random() })
      }
