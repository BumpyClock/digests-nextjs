...
export class IndexedDBAdapter implements PersistenceAdapter {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.storeName)) {
          ...
          store.createIndex('dataUpdatedAt', 'dataUpdatedAt', { unique: false })
  ...
  private startAutomaticCleanup(): void {
    ...
    this.cleanupInterval = setInterval(async () => {
      try {
        ...
        // Clean up if needed
        if (health.percentage > this.maxStoragePercent || health.hasExpiredItems) {
   ...
   */
  private handleError(error: unknown, type: PersistenceErrorType): PersistenceError {
