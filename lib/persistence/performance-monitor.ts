...
export class PersistenceMonitor {
  ...
  private recordLatency(type: 'read' | 'write', duration: number) {
    const latencies = type === 'read' ? this.readLatencies : this.writeLatencies
    ...
    if (duration > 100 && process.env.NODE_ENV === 'development') {
      console.warn(`[Persistence] Slow ${type} operation: ${duration.toFixed(1)}ms`)
    }
  }
  ...
  private async updateStorageMetrics() {
    ...
    if ('getItemCount' in this.adapter && typeof this.adapter.getItemCount === 'function') {
      try {
  ...
  getSummary() {
    ...
    const totalOps = metrics.operations.reads + metrics.operations.writes + metrics.operations.deletes
    const errorRate = totalOps > 0 ? (metrics.operations.errors / totalOps) * 100 : 0
    const storageUsagePercent = metrics.storage.quota > 0 
    ...
    return {
      ...
      throttleRate: metrics.operations.writes > 0 
        ? ((metrics.throttling.throttledWrites / metrics.operations.writes) * 100).toFixed(1) + '%'
        : '0%',
...
export function createMonitoredAdapter(
  ...
  return {
    ...
    async set<T>(key: string, value: T, ttl?: number): Promise<void> {
      return monitor.monitorOperation('write', () => adapter.set(key, value, ttl))
    },
    ...
    async setMany<T>(items: Map<string, T>, ttl?: number): Promise<void> {
      ...
      return monitor.monitorOperation('write', () => adapter.setMany(items, ttl))
    },

    async deleteMany(keys: string[]): Promise<void> {
      return monitor.monitorOperation('delete', () => adapter.deleteMany(keys))
    },

    async keys(pattern?: string): Promise<string[]> {
      return monitor.monitorOperation('read', () => adapter.keys(pattern))
    },
