...
class LocalStorageProvider implements StorageProvider {
  ...
  async getItem<T>(key: string): Promise<T | null> {
    try {
      ...
      Logger.error(
        `Error getting item from storage: ${key}`,
        error as Error,
        this.LOG_CONTEXT,
      );
      return null;
  ...
  async setItem<T>(key: string, value: T): Promise<void> {
    try {
      ...
      Logger.error(
        `Error setting item in storage: ${key}`,
        error as Error,
        this.LOG_CONTEXT,
      );
  ...
  async removeItem(key: string): Promise<void> {
    try {
      ...
      Logger.error(
        `Error removing item from storage: ${key}`,
        error as Error,
        this.LOG_CONTEXT,
      );
...
export class StorageService {
  ...
  async getFeeds(): Promise<Result<Feed[]>> {
    try {
      const storage = await this.provider.getItem<FeedStorage>(
        STORAGE_KEYS.FEEDS,
      );
      ...
      Logger.error(
        "Error getting feeds from storage",
        error as Error,
        this.LOG_CONTEXT,
      );
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to get feeds from storage"),
      };
    }
  }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to save feeds to storage"),
      };
    }
  }
  ...
  async getFeedItems(): Promise<Result<FeedItem[]>> {
    try {
      const storage = await this.provider.getItem<FeedItemStorage>(
        STORAGE_KEYS.FEED_ITEMS,
      );
      ...
      Logger.error(
        "Error getting feed items from storage",
        error as Error,
        this.LOG_CONTEXT,
      );
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to get feed items from storage"),
      };
    }
  }
  ...
  async saveFeedItems(items: FeedItem[]): Promise<Result<void>> {
    try {
      ...
      Logger.error(
        "Error saving feed items to storage",
        error as Error,
        this.LOG_CONTEXT,
      );
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to save feed items to storage"),
      };
    }
  }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to add feed to storage"),
      };
    }
  }
  ...
  async removeFeed(feedUrl: string): Promise<Result<void>> {
    try {
      ...
      const feeds = (feedsResult.data || []).filter(
        (f) => f.feedUrl !== feedUrl,
      );
      ...
      Logger.error(
        "Error removing feed from storage",
        error as Error,
        this.LOG_CONTEXT,
      );
      return {
        ...
        error:
          error instanceof Error
            ? error
            : new StorageError("Failed to remove feed from storage"),
      };
    }
  }
}
