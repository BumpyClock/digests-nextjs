...
describe('WorkerService', () => {
  ...
  describe('RSS Parsing', () => {
    it('should parse RSS feeds', async () => {
      ...
      const result = await workerService.parseRSS(xmlContent, 'https://example.com/feed');
    ...
    it('should handle Atom feeds', async () => {
      ...
      const result = await workerService.parseRSS(atomContent, 'https://example.com/atom');
    ...
    it('should handle podcast feeds', async () => {
      ...
      const result = await workerService.parseRSS(podcastXml, 'https://example.com/podcast');
  ...
  describe('Batch Processing', () => {
    ...
    it('should handle partial failures', async () => {
      ...
      expect(results.filter(r => r.error === undefined)).toHaveLength(2);
  ...
  describe('Content Extraction', () => {
    it('should extract full content when available', async () => {
      ...
      const result = await workerService.parseRSS(xmlWithContent, 'https://example.com/feed');

      expect(result.items[0].content_html).toContain('Full HTML content');
      expect(result.items[0].content_html).toContain('<strong>formatting</strong>');
    });
    ...
    it('should fallback to description if no content', async () => {
      ...
      const result = await workerService.parseRSS(xmlWithDescription, 'https://example.com/feed');
  ...
  describe('Feed Metadata', () => {
    it('should extract feed metadata', async () => {
      ...
      const result = await workerService.parseRSS(xmlWithMetadata, 'https://techblog.com/feed');

      const feed = result.feeds[0];
      expect(feed.title).toBe('Tech Blog');
      expect(feed.site_url).toBe('https://techblog.com');
      expect(feed.description).toBe('Latest tech news');
      expect(feed.metadata?.language).toBe('en-US');
      expect(feed.metadata?.image).toBe('https://techblog.com/logo.png');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should timeout long-running operations', async () => {
      ...
      await expect(
        workerService.parseRSS('<rss></rss>', 'https://example.com/slow', { timeout: 100 })
      ...
      crashingWorker.postMessage = jest.fn(() => {
        setTimeout(() => {
          if (crashingWorker.onerror) {
            crashingWorker.onerror(new ErrorEvent('error', {
