...
describe('ApiService', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should update API URL and clear cache', () => {
      ...
      expect(Logger.debug).toHaveBeenCalledWith('[ApiService] API URL updated to http://new-api.example.com')
    })
    ...
    it('should update cache TTL', () => {
      ...
      expect(Logger.debug).toHaveBeenCalledWith('[ApiService] Cache TTL updated to 60000ms')
    })
  })
  ...
  describe('fetchFeeds', () => {
    ...
    it('should return cached results on subsequent calls', async () => {
      ...
      const result1 = await apiService.fetchFeeds(['http://example.com/feed.xml'])
      expect(mockFetch).toHaveBeenCalledTimes(1)

      // Second call - should return from cache
      const result2 = await apiService.fetchFeeds(['http://example.com/feed.xml'])
  ...
  describe('refreshFeeds', () => {
    ...
    it('should clear cache and fetch fresh data', async () => {
      ...
      expect(deleteSpy).toHaveBeenCalledWith('feeds:http://example.com/feed.xml')
    ...
    it('should handle refresh errors', async () => {
      ...
      await expect(apiService.refreshFeeds(['http://example.com/feed.xml']))
  ...
  describe('fetchReaderView', () => {
    ...
    it('should fetch reader view successfully', async () => {
      ...
      const result = await apiService.fetchReaderView('http://example.com/article')
    ...
    it('should return cached reader view on subsequent calls', async () => {
      ...
      const result1 = await apiService.fetchReaderView('http://example.com/article')
      expect(mockFetch).toHaveBeenCalledTimes(1)

      const result2 = await apiService.fetchReaderView('http://example.com/article')
      expect(mockFetch).toHaveBeenCalledTimes(1)
      expect(result2).toEqual(result1)
      expect(Logger.debug).toHaveBeenCalledWith(expect.stringContaining('Cache hit'))
    })
    ...
    it('should handle empty reader view response', async () => {
      ...
      await expect(apiService.fetchReaderView('http://example.com/article'))
        .rejects.toThrow('Invalid response from API')
    })
  ...
  describe('Feed operations', () => {
    ...
    describe('getById', () => {
      ...
      it('should throw error if feed not found', async () => {
        ...
        await expect(apiService.feeds.getById('non-existent'))
          .rejects.toThrow('Feed with id non-existent not found')
      })
    })
    ...
    describe('create', () => {
      it('should create a new feed', async () => {
        ...
        const feed = await apiService.feeds.create({ url: 'http://example.com/feed.xml' })
        expect(feed).toEqual(mockFeed)
      })
      ...
      it('should throw error if feed creation fails', async () => {
        ...
        await expect(apiService.feeds.create({ url: 'http://example.com/feed.xml' }))
    ...
    describe('update', () => {
      it('should throw not implemented error', async () => {
        await expect(apiService.feeds.update('feed-1', { feedTitle: 'New Title' }))
  ...
  describe('Article operations', () => {
    ...
    describe('markAsRead', () => {
      it('should mark article as read (client-side)', async () => {
        await expect(apiService.articles.markAsRead('item-1')).resolves.toBeUndefined()
      })
    })

    describe('markAsUnread', () => {
      it('should mark article as unread (client-side)', async () => {
        await expect(apiService.articles.markAsUnread('item-1')).resolves.toBeUndefined()
      })
    })
  })
