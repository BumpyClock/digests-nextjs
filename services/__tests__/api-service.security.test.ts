...
describe('ApiService Security Tests', () => {
  describe('Cache Key Generation', () => {
    it('should generate SHA-256 hashed cache keys for feed URLs', async () => {
      ...
      const expectedHash = crypto.createHash('sha256').update(`feeds:${sortedUrls}`).digest('hex');
    ...
    it('should generate consistent cache keys for same URLs in different order', async () => {
      ...
      const expectedHash = crypto.createHash('sha256').update(sortedUrls).digest('hex');
  ...
  describe('URL Validation', () => {
    it('should accept valid HTTP URLs', () => {
      expect(() => apiService.updateApiUrl('http://localhost:3000')).not.toThrow();
      expect(() => apiService.updateApiUrl('https://api.example.com')).not.toThrow();
    });

    it('should reject invalid URLs', () => {
      expect(() => apiService.updateApiUrl('not-a-url')).toThrow('Invalid API URL');
      expect(() => apiService.updateApiUrl('')).toThrow('Invalid API URL');
      expect(() => apiService.updateApiUrl('javascript:alert(1)')).toThrow('Invalid protocol');
      expect(() => apiService.updateApiUrl('file:///etc/passwd')).toThrow('Invalid protocol');
      expect(() => apiService.updateApiUrl('ftp://example.com')).toThrow('Invalid protocol');
    });
