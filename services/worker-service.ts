...
type WorkerMessage =
  ...
  | {
      type: "PARSE_RSS_CONTENT";
      payload: { xmlContent: string; url: string; apiBaseUrl?: string };
    }
...
class WorkerService {
  ...
  /**
   * Parses RSS content directly from XML string
   */
  async parseRSS(
    xmlContent: string,
    url: string,
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this.rssWorker) {
        console.warn("WorkerService: Worker not available, using fallback");
        // For fallback, we'd need a direct RSS parser
        // For now, reject since we don't have direct parsing capability
        reject(new Error("RSS parsing not available without worker"));
        return;
      }
      ...
      const unsubscribe = this.onMessage("FEEDS_RESULT", (response) => {
        ...
        if (response.success) {
          ...
          reject(new Error(response.message || "RSS parsing failed"));
        }
      });

      // Send XML content to worker for parsing
      this.postMessage({
        type: "PARSE_RSS_CONTENT",
        payload: {
          xmlContent,
  ...
  /**
   * Processes multiple feed URLs in batch
   */
  async processFeedBatch(urls: string[]): Promise<
    Array<{
      url: string;
      feeds: Feed[];
      items: FeedItem[];
  ...
  > {
    ...
    const promises = urls.map(async (url) => {
      try {
        const result = await this.fetchFeeds(url);
        return {
          url,
          feeds: result.feeds,
          items: result.items,
