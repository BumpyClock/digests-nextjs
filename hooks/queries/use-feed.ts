...
export const useFeed = (
  feedId: string,
  options?: Omit<
    UseQueryOptions<SingleFeedQueryData>,
  ...
  return useQuery<SingleFeedQueryData>({
    ...
    queryFn: async () => {
      ...
      const cachedData = queryClient.getQueryData<FeedsQueryData>(
        feedsKeys.lists(),
      );
      ...
      if (cachedFeed && cachedData) {
        ...
        const feedItems = cachedData.items.filter(
          (item) => item.feedUrl === cachedFeed.feedUrl,
        );
    ...
    networkMode: "online", // Offline support disabled during migration
    ...options,
  });
};
...
export const useFeedItems = (
  ...
  return useQuery<FeedItem[]>({
    ...
    enabled: !!feedUrl && true, // React Query feeds permanently enabled
