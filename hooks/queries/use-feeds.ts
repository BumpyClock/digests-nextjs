...
export const feedsKeys = {
  ...
  itemsByFeed: (feedUrl: string) => [...feedsKeys.items(), { feedUrl }] as const,
 ...
 */
export const useFeeds = (options?: UseQueryOptions<FeedsQueryData>) => {
  const queryClient = useQueryClient()
  const { feeds: zustandFeeds, setFeeds, setFeedItems } = useFeedStore()
  const isFeatureEnabled = FEATURES.USE_REACT_QUERY_FEEDS
  
  // Determine data source based on feature flag
  const existingFeeds = isFeatureEnabled 
  ...
  const query = useQuery<FeedsQueryData>({
    ...
    refetchInterval: FEATURES.ENABLE_BACKGROUND_SYNC ? 30 * 60 * 1000 : false, // 30 minutes if background sync enabled
    refetchIntervalInBackground: FEATURES.ENABLE_BACKGROUND_SYNC,
  ...
  // Sync with Zustand if not using React Query
  useEffect(() => {
    if (!isFeatureEnabled && query.isSuccess && query.data) {
      setFeeds(query.data.feeds)
      setFeedItems(query.data.items)
    }
  }, [isFeatureEnabled, query.isSuccess, query.data, setFeeds, setFeedItems])

  return {
    ...query,
    feeds: isFeatureEnabled ? (query.data?.feeds || []) : zustandFeeds,
    items: isFeatureEnabled ? (query.data?.items || []) : [],
 ...
 */
export const useFeed = (feedId: string, options?: UseQueryOptions<SingleFeedQueryData>) => {
  const queryClient = useQueryClient()
  ...
  return useQuery<SingleFeedQueryData>({
    ...
    queryFn: async () => {
      ...
      if (cachedFeed) {
        ...
        return { feed: cachedFeed, items: feedItems }
      }
