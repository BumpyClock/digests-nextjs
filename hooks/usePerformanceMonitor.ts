...
// Note: performanceMiddleware was removed during cleanup - create local metrics tracking
...
export function usePerformanceMonitor(enabled = true) {
  ...
  const checkForAlerts = useCallback((sample: PerformanceSample) => {
    ...
    if (sample.memory && samplesRef.current.length > 5) {
      ...
      const avgMemory =
        recentSamples
          .filter((s) => s.memory)
          .reduce((sum, s) => sum + (s.memory?.usedJSHeapSize || 0), 0) /
        recentSamples.length;

      if (sample.memory.usedJSHeapSize > avgMemory * 1.1) {
        // 10% spike
        alerts.push({
          type: "MEMORY_SPIKE",
          severity:
            sample.memory.usedJSHeapSize > avgMemory * 1.2 ? "HIGH" : "MEDIUM",
  ...
  const getPerformanceSummary = useCallback(() => {
    ...
    const avgQueries =
      samples.reduce((sum, s) => sum + s.cache.queryCount, 0) / samples.length;
    const avgActiveQueries =
      samples.reduce((sum, s) => sum + s.cache.activeQueries, 0) /
      samples.length;
...
export function useComponentRenderMonitor(componentName: string) {
  ...
  useEffect(() => {
    ...
    if (renderTime > 16 && renderCount.current > 1) {
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          `[Slow Render] ${componentName} took ${renderTime.toFixed(1)}ms to render`,
        );
      }
    }
    ...
    if (
      ...
      console.log(
        `[Render Stats] ${componentName}: ${renderCount.current} renders, avg ${avgRenderTime.toFixed(1)}ms`,
      );
    }
  });
