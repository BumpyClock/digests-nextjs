...
interface UsePersistentQueryOptions<TData, TError = unknown>
  extends Omit<UseQueryOptions<TData, TError>, "queryKey" | "queryFn" | "meta">,
    QueryPersistOptions {
  /**
...
export function usePersistentQuery<TData = unknown, TError = unknown>(
  ...
  const syncStatusRef = useRef<"idle" | "syncing" | "success" | "error">(
    "idle",
  );
  ...
  const enhancedQueryFn = useCallback(async () => {
    ...
    if (!isOnline && queryOptions.enabled !== false) {
      const cachedData =
        queryOptions.initialData || queryOptions.placeholderData;
      if (cachedData) {
  ...
  const query = useQuery({
    ...
    refetchIntervalInBackground: isOnline
      ? queryOptions.refetchIntervalInBackground
      : false,
    // Custom meta for persistence - handled by React Query persister
    // meta: {
    //   ...queryOptions.meta,
    //   ...persistOptions,
    // },
