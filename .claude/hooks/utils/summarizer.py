...
from .llm_client import call_llm_with_fallback, _log_to_file


def generate_event_summary(event_data: Dict[str, Any], hook_name: Optional[str] = None) -> Optional[str]:
    """
    ...
    Args:
        ...
        hook_name: Optional name/identifier of the hook that called this function

    Returns:
        str: A one-sentence summary, or None if generation fails
    """
    hook_caller = hook_name or "unknown-hook"
    _log_to_file(f"=== SUMMARIZER CALLED by {hook_caller} ===")
...
Generate the summary based on the payload:"""
    ...
    summary, provider_used = call_llm_with_fallback(prompt, mode='prompt')
    ...
    if summary:
        ...
        _log_to_file(f"Summary generated using {provider_used}: {summary}")
    else:
        _log_to_file("No summary generated")

    _log_to_file("=== SUMMARIZER COMPLETE ===")
    # If LLM fails, provide a simple fallback summary
    if not summary:
        event_type = event_data.get("hook_event_type", "Unknown")
        templates = {
        ...
        summary = templates.get(event_type, templates["Unknown"])
