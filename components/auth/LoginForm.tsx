// ABOUTME: Modern login form component with React Query auth integration
// ABOUTME: Provides secure login with validation, error handling, and optimistic updates
...
export function LoginForm({
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError.type === 'INVALID_CREDENTIALS') {
      ...
      } else if (authError.type === 'EMAIL_NOT_VERIFIED') {
        setFormErrors({
        ...
        setFormErrors({
          general: authError.message || 'An error occurred during login. Please try again.'
        })
      }
  ...
  const isFormDisabled = disabled || isLoggingIn
...
/**
 * Minimal login form for modals/dropdowns
 */
export function CompactLoginForm({ onSuccess, onError, className }: LoginFormProps) {
  return (
    <LoginForm
      title=""
      description=""
      ...
      onSuccess={onSuccess}
      onError={onError}
      className={cn("shadow-none border-0", className)}
    />
  )
}

/**
 * Login form with social auth options (placeholder for future)
 */
export function SocialLoginForm(props: LoginFormProps) {
  return (
