...
export function PerformanceMonitor({
  ...
  const hasMemoryAlert = alerts.some(
    (a) => a.type === "MEMORY_SPIKE" && Date.now() - a.timestamp < 30000,
  );
  const hasCacheAlert = alerts.some(
    (a) => a.type === "CACHE_MISS_RATE" && Date.now() - a.timestamp < 30000,
  );
    ...
    <div className={`fixed ${positionClasses[position]} z-50 w-80`}>
      <div className="bg-black/90 backdrop-blur-sm rounded-lg shadow-2xl text-white p-4 font-mono text-xs">
        ...
        <div className="mb-3">
          ...
          <div className="space-y-1">
            <div className="flex justify-between">
              <span>Current:</span>
              <span className={hasMemoryAlert ? "text-red-400" : ""}>
                {memoryUsageMB} MB
              </span>
            </div>
            ...
            <div className="w-full bg-gray-800 rounded-full h-1.5 mt-1">
              <div
                ...
                style={{
                  width: `${Math.min((summary.memory.current / summary.memory.peak) * 100, 100)}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="mb-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div>
              ...
              <span className="ml-1">
                {Math.round(summary.cache.averageActive)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 pt-2 border-t border-gray-700 text-xs text-gray-400">
          <div>
            Samples: {summary.samples} | Alerts: {summary.alerts}
          </div>
        </div>
      </div>
    </div>
