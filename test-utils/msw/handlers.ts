...
import { createMockFeeds, createMockFeedItems, createMockArticle } from '../factories';
...
export const handlers = [
  ...
  http.post(`${API_BASE_URL}/parse`, async ({ request }: { request: Request }) => {
    ...
    body.urls.forEach((url, index) => {
      ...
      apiFeed.items = mockItems.map(item => convertToApiFeedItem(item, apiFeed));
  ...
  http.post(`${API_BASE_URL}/getreaderview`, async ({ request }: { request: Request }) => {
    const body = await request.json() as { urls: string[] };
  ...
  http.get(`${API_BASE_URL}/api/feeds/:feedId/items`, ({ params }: { params: Record<string, string> }) => {
    const { feedId } = params;
    const mockFeed = createMockFeeds(1, { guid: feedId as string })[0];
    const apiFeed = convertToApiFeed(mockFeed);
    const mockItems = createMockFeedItems(10, feedId as string);
    const apiItems = mockItems.map(item => convertToApiFeedItem(item, apiFeed));
    return HttpResponse.json({ items: apiItems });
  }),

  // GET /api/reader/:itemId - Get reader view of an article
  http.get(`${API_BASE_URL}/api/reader/:itemId`, ({ params }: { params: Record<string, string> }) => {
    const { itemId } = params;
    const article = createMockArticle({ url: `https://example.com/article-${itemId}` });
  ...
  http.post(`${API_BASE_URL}/api/feeds/:feedId/refresh`, () => {
    return HttpResponse.json({ success: true, message: 'Feed refreshed successfully' });
  }),

  // POST /api/feeds - Add a new feed
  http.post(`${API_BASE_URL}/api/feeds`, async ({ request }: { request: Request }) => {
