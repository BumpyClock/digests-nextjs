...
export const SettingsFeedCard = memo(function SettingsFeedCard({
  ...
  const placeholderImage =
    feed.type === "podcast" ? placeholderPodcast : placeholderRss;

  return (
    <div className="relative group transition-all duration-200 group-hover:scale-105 group-hover:translate-y-[-8px]">
      <div
        id={`settings-feed-card-${feed.feedUrl}`}
        className="relative p-5 flex flex-col h-full z-2"
      >
        <div className="flex-1">
          ...
          {feed.feedTitle && (
            <p className="text-caption text-sm mb-2">{feed.feedTitle}</p>
          )}
      ...
      <div className="overflow-hidden rounded-2xl absolute top-0 left-0 w-full h-full z-[-1]">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${feed.favicon || placeholderImage})`,
          }}
        />
      </div>
