  ...
  useEffect(() => {
    const fetchFeedDetails = async () => {
      ...
      try {
        ...
        // Direct API call - no worker overhead
        ...
        if (result.feeds.length > 0) {
          ...
          const updatedFeeds = initialFeeds.map((feed) => {
            const fetchedFeed = result.feeds.find(
            ...
            if (fetchedFeed) {
              return {
                ...
                feed: fetchedFeed,
                title:
                  fetchedFeed.feedTitle ||
                  fetchedFeed.siteTitle ||
                  feed.title ||
                  feed.url,
              };
            }
          ...
          // If no feeds were returned, mark all feeds as errored
          const updatedFeeds = initialFeeds.map((feed) => ({
            ...feed,
