...
export class EncryptionKeyManager {
  ...
  static async retrieveKey(
    ...
    try {
      ...
      const exported = await new Promise<JsonWebKey | null>(
        (resolve, reject) => {
...
export class EncryptionUtils {
  ...
  static async encrypt(
    ...
    const algorithm = config?.algorithm || "AES-GCM";
    const iv =
      config?.generateIV?.() || crypto.getRandomValues(new Uint8Array(12));
...
export class EncryptedPersistenceAdapter {
  ...
  async get<T>(key: string): Promise<T | null> {
    const encrypted = (await this.adapter.get(key)) as string;
  ...
  async getMany<T>(keys: string[]): Promise<Map<string, T>> {
    const encrypted = (await this.adapter.getMany(keys)) as Map<string, string>;
    ...
    for (const [key, value] of encrypted) {
      if (value) {
        try {
          ...
          console.error(
            "Batch decryption failed for key:",
            key,
            error instanceof Error ? error.message : String(error),
          );
        }
