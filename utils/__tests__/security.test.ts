...
describe("Security Utilities", () => {
  ...
  describe("sanitizeHtml", () => {
    it("should escape HTML special characters", () => {
      ...
      expect(sanitizeHtml("a < b && c > d")).toBe(
        "a &lt; b &amp;&amp; c &gt; d",
      );
    });
  ...
  describe("validateFeedUrls", () => {
    ...
    it("should limit number of URLs processed", () => {
      const urls = Array.from(
        { length: 150 },
        (_, i) => `https://example${i}.com/feed`,
      );
      const result = validateFeedUrls(urls);

      expect(result.valid.length + result.invalid.length).toBe(
        SECURITY_CONFIG.MAX_FEED_URLS,
      );
    });
  ...
  describe("SECURITY_CONFIG", () => {
    it("should have expected configuration values", () => {
      ...
      expect(SECURITY_CONFIG.BLOCKED_HOSTNAMES).toEqual([
        "localhost",
        "127.0.0.1",
      ]);
    });
