...
export function mergeFeedData(
  ...
  Logger.debug(
    "[PersistenceHelpers] Merging feed data with strategy:",
    strategy,
  );
  ...
  switch (strategy) {
    ...
    default:
      Logger.warn(
        "[PersistenceHelpers] Unknown strategy, using merge-by-timestamp",
      );
      return mergeByTimestamp(localData, remoteData);
  }
}
...
function mergeByTimestamp(
  ...
  if (useRemote && remoteData.feeds && remoteData.items) {
    return {
      ...
      readItems: mergeReadItems(
        localData.readItems,
        remoteData.readItems || [],
      ),
      metadata: {
...
export function migrateData(
  ...
  Logger.debug(
    "[PersistenceHelpers] Migrating data from version",
    fromVersion,
    "to",
    toVersion,
  );
...
export function calculateDataSize(data: PersistedFeedData): number {
  try {
    ...
    Logger.error(
      "[PersistenceHelpers] Failed to calculate data size:",
...
export function compressData(data: PersistedFeedData): string {
  try {
    ...
    Logger.error(
      "[PersistenceHelpers] Failed to compress data:",
      error instanceof Error ? error : new Error(String(error)),
    );
...
export function decompressData(compressed: string): PersistedFeedData {
  try {
    ...
    if (!validateFeedData(data)) {
      Logger.warn(
        "[PersistenceHelpers] Invalid data structure, using empty data",
      );
    ...
    Logger.error(
      "[PersistenceHelpers] Failed to decompress data:",
      error instanceof Error ? error : new Error(String(error)),
    );
    return createEmptyFeedData();
  }
...
export function cleanupOldItems(
  ...
  const cleanedReadItems = data.readItems.filter((id) =>
...
export const debugUtils = {
  ...
  validateIntegrity: (data: PersistedFeedData): boolean => {
    ...
    const orphanedItems = Array.from(itemFeedUrls).filter(
      (url) => !feedUrls.has(url),
    );

    if (orphanedItems.length > 0) {
      Logger.warn(
        "[PersistenceHelpers] Found orphaned items for feeds:",
        orphanedItems,
      );
      return false;
    }
    ...
    if (orphanedReadItems.length > 0) {
      Logger.warn(
        "[PersistenceHelpers] Found orphaned read items:",
        orphanedReadItems.length,
      );
      return false;
    }
